package com.example.dailycarebe.hash;
...
public class HashIds {
    ...
    public String encodeNumber(Long number) {
        return number == null ? null : encode(number);
    }

    public long decodeNumber(String hashId) {
        if (StringUtils.isEmpty(hashId))
            return NOT_EXIST_ID;
        long[] decodedNumbers = decode(hashId);
        return decodedNumbers.length == 0 ? -1 : decodedNumbers[0];
    }

    public String encodeNumbers(long... numbers) {
        return encode(numbers);
    }

    public long[] decodeNumbers(String hashId) {
        return decode(hashId);
    }

    public HashIds(@Nonnull String salt, @Nonnull String alphabet, @Nonnull String seps, int minHashLength) {
        this.salt = salt;
